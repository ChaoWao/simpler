...
renders only its own steps. The main CI keeps the onboard NPU DFX smokes in
the base onboard scene-test jobs rather than in separate follow-on jobs.
Shared step scaffolding that is safe to run after checkout lives in composite
actions under `.github/actions/` (`cache-pip`, `setup-venv`, and the three
`pod-*` actions).
...
- **Gated jobs (scene tests):** `st-sim-{a2a3,a5}`, `st-onboard-{a2a3,a5}`, and `st-pod-onboard-a2a3` run iff their platform's flag **and** `st_affected` are `true`.
