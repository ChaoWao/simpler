...
- [2026-08 — HBG qwen3-14B host bind decomposition: orchestration is cheap, SVM unmap is not](2026-08-15-hbg-qwen3-host-bind-strace.md) — ten fine-grained `[STRACE]` spans on the HBG bind path (identical on a2a3/a5), 50 rounds on an exclusive even die: graph record/replay is **~2.9 ms/step** and post-graph H2D ~1.8 ms (both negligible), but `bind.host_orch.tensor_view_unmap` — serial `halHostUnregister` of every staged tensor's host view — costs **~256 ms/step (12.6% of host wall)**, and `bind.args` re-staging unchanged tensors costs 1.26 s (62%). The a5 rerun (2026-08-17) shows `bind.args` dominating even harder (**7.1 s, 72.3%**, host↔device copies several x slower across every transfer span) while `tensor_view_unmap` is **~0 by construction** — a5 onboard has no host-map path (`register_device_memory_to_host` returns nullptr), so the mapping-reuse direction is a2a3-only. Optimization directions recorded, not pursued
