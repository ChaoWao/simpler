...
- [2026-08 — Arbitrary-depth WAIT-edge reduction on tmr: bounded bitmap vs the 1-hop ceiling](2026-08-tmr-bounded-wait-reach-bitmap.md) — shipped as the BL=64 reachability bitmap (`reduce_wait_edges`, #1376): frozen per-task 64-bit ancestor word + the submit-claim pin overturn the earlier "no reliable view past one hop" premise that scoped the abandoned 1-hop PR #1830, at O(fanin) word ops per submit with no inline cap (vs #1830's O(count²·fanin) and skipped dense fanins). Representative replay: paged_attention_unroll's upper bound 256 redundant edges, BL=64 removes 100%. `wait_reduction_sim` ships with the wheel so the BL choice stays measurable
