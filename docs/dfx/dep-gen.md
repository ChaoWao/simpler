...
keep the full annotation. The `flags` field is **per record**: the
runtime OR-accumulates fanin flags per `(pred, succ)` pair, so a pair
can carry one record with `["wait","retain"]` (a `creator` edge) and
another with `["wait"]` (a `tensormap` edge). A consumer reconstructing
the pair's aggregate dependency semantics must OR `flags` across all
records of that pair.
