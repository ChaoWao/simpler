...
# --rtt-json mode, validates every measurement, and atomically writes the plan
# (plus aicpu_affinity_plan.<device_id>.cpus for the thin runtime reader).
# Ranking/packing and pool<5 / probe-failure contiguous fallbacks are owned
# exclusively by the Python tool; production DeviceRunner only reads the plan
# (OCCUPY contiguous if the .cpus file is still missing).
...
FG/PG selection for `ALLOWED_CPUS`; `--json` is for tooling/diagnostics and for
Python's probe-failure contiguous fallback (`--fallback-occupy` / auto).

`--rtt-json` runs the full affinity preflight (schema v3):

1. serial `aicpu_num=1` enumeration of the user pool
...
Pool size `< 5` emits `pool_too_small` and skips handshake/COND. Launch uses
the OCCUPY popcount so the gate sees the full user pool.
