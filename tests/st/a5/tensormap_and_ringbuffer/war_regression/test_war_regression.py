...
# -----------------------------------------------------------------------------------------------------------
"""A5 loop-carried and cross-ring write-after-read regressions."""
...
class TestWarRegressionTmrA5(SceneTestCase):
    RTOL = 0
    ATOL = 0
    CALLABLE = {
        "orchestration": {
    ...
    def _validate_war_edge(self, case, run_marker):
        label = _sanitize_for_filename(f"TestWarRegressionTmrA5_{case['name']}")
