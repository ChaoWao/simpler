...
# -----------------------------------------------------------------------------------------------------------
"""A5 HBG loop-carried write-after-read regression."""
...
class TestWarRegressionHbgA5(SceneTestCase):
    RTOL = 0
    ATOL = 0
    CALLABLE = {
        "orchestration": {
    ...
    def _validate_war_edge(self, case, run_marker):
        label = _sanitize_for_filename(f"TestWarRegressionHbgA5_{case['name']}")
