...
# -----------------------------------------------------------------------------------------------------------
"""Pod collection sentinel for tests/st.

This intentionally re-runs the global_tload_mixed_l3 example path so pod CI
proves both `examples/` and `tests/st/` pod tests are collected by marker.
"""
...
def test_global_tload_mixed_l3_pod(st_platform, st_device_ids, st_pod_peer, st_pod_remote_device_ids, st_pod_logs):
    rc = run(
