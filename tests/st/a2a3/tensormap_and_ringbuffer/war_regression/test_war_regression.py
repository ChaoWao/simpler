...
# -----------------------------------------------------------------------------------------------------------
"""A2/A3 loop-carried and cross-ring write-after-read regressions."""
...
class TestWarRegressionTmrA2A3(SceneTestCase):
    RTOL = 0
    ATOL = 0
    CALLABLE = {
        "orchestration": {
    ...
    CASES = [
        ...
        },
        {
            "name": "CrossRing",
            "platforms": ["a2a3sim", "a2a3"],
            "params": {"cross_ring": True, "war_pred": 1 << 32, "war_succ": 1},
        },
    ]
    ...
    def _validate_war_edge(self, case, run_marker):
        label = _sanitize_for_filename(f"TestWarRegressionTmrA2A3_{case['name']}")
