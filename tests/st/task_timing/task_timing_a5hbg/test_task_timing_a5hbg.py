...
def test_a5hbg_task_timing_slots_emit_markers(st_platform, st_device_ids, capfd):
    ...
    try:
        ...
        # as a FORK_SHM Tensor via make_tensor_arg. a, b are inputs; out is where the hbg runtime's D2H
        # lands (in-process, so the parent sees it — no share_memory_ needed at L2).
        args = TaskArgs()
        _f32 = DataType.FLOAT32
        args.add_tensor(worker.make_tensor_arg(a, shapes=(_SIZE,), dtype=_f32), TensorArgType.INPUT)
        args.add_tensor(worker.make_tensor_arg(b, shapes=(_SIZE,), dtype=_f32), TensorArgType.INPUT)
        args.add_tensor(worker.make_tensor_arg(out, shapes=(_SIZE,), dtype=_f32), TensorArgType.OUTPUT_EXISTING)
