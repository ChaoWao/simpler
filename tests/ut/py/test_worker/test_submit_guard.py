...
import pytest
from simpler.buffer_handle import AccessMode, mint_owner_instance_id, wrap_fork_inherited
...
def _rw_handle(nbytes=256):
    return wrap_fork_inherited(0x20000, nbytes, _OID, buffer_id=2, access=AccessMode.READWRITE)
...
def test_same_offsets_on_different_backings_are_legal():
    a, b = _rw_handle(), wrap_fork_inherited(0x30000, 256, _OID, buffer_id=3, access=AccessMode.READWRITE)
...
def test_fork_cow_handle_cannot_claim_a_write_grant():
    ...
    from simpler.buffer_handle import BackendKind, BufferHandleDescriptor  # noqa: PLC0415
