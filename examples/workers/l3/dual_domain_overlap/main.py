    ...
    try:
            ...
            def _orch_fn(orch, _args, cfg):
                ...
                with orch.allocate_domain(
                    ...
                    for worker_idx in worker_indices:
                        ...
                        args = TaskArgs()
                        args.add_tensor(make_tensor(worker, host_x[worker_idx]), TensorArgType.INPUT)
                        args.add_tensor(
                            make_tensor(worker, reduce_out[domain_name][worker_idx]), TensorArgType.OUTPUT_EXISTING
                        )
        ...
        def affine_orch_fn(orch, _args, cfg):
            for domain_name, worker_indices in DOMAINS.items():
                ...
                for worker_idx in worker_indices:
                    args = TaskArgs()
                    args.add_tensor(make_tensor(worker, reduce_out[domain_name][worker_idx]), TensorArgType.INPUT)
                    args.add_tensor(make_tensor(worker, scale[worker_idx]), TensorArgType.INPUT)
                    args.add_tensor(make_tensor(worker, bias[worker_idx]), TensorArgType.INPUT)
                    args.add_tensor(
                        make_tensor(worker, affine_out[domain_name][worker_idx]), TensorArgType.OUTPUT_EXISTING
                    )
