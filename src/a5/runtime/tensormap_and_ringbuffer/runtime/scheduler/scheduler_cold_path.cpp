...
void SchedulerContext::log_stall_diagnostics(
    ...
    const int32_t cluster_begin = (ast > 0) ? thread_idx * aic_count_ / ast : 0;
...
void SchedulerContext::handshake_owned_clusters(Runtime *runtime, int32_t tidx, int32_t active_threads) {
    ...
    const int32_t begin = (active_threads > 0) ? tidx * aic_n / active_threads : 0;
    const int32_t end = (active_threads > 0) ? (tidx + 1) * aic_n / active_threads : 0;
...
void SchedulerContext::assign_own_clusters(int32_t tidx) {
    ...
    const int32_t begin = (active > 0) ? tidx * aic_n / active : 0;
    const int32_t end = (active > 0) ? (tidx + 1) * aic_n / active : 0;
...
bool SchedulerContext::assign_cores_to_threads() {
    ...
    for (int32_t t = 0; t < active; t++) {
        const int32_t begin = t * cluster_count / active;
        const int32_t end = (t + 1) * cluster_count / active;
