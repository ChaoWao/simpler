...
void PTO2TensorMap::print_stats() {
    ...
    auto collect_bucket_stats = [&](PTO2TensorMapEntry **index, int32_t bucket_count) {
        BucketStats stats;
    ...
    const BucketStats writer_stats = collect_bucket_stats(buckets, num_buckets);
    const BucketStats reader_stats = collect_bucket_stats(reader_buckets, num_reader_buckets);
    ...
    LOG_INFO("Reader entries:      live=%d high_water=%d", reader_used, reader_high_water);
    LOG_INFO("Writer entries:      live=%d", current_writers());
    LOG_INFO("Buckets:             writer=%d reader=%d", num_buckets, num_reader_buckets);
    ...
    LOG_INFO("Last task alive:     %d", last_task_alive_cached);
    LOG_INFO("============================");
}
