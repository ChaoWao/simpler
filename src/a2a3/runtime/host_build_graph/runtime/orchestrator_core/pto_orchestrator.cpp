...
std::byte *graph_host_pinned_bump(size_t bytes) {
    ...
    const size_t off = (g_pin_used + kGraphPinnedBumpAlign - 1) & ~(kGraphPinnedBumpAlign - 1);
...
static bool graph_submission_layout(
    ...
    if (total_bytes == nullptr || tensors_offset == nullptr || scalars_offset == nullptr || scalar_bytes == nullptr ||
        graph_definition(definition_image) == nullptr) {
        return false;
    }
    *tensors_offset = PTO2_ALIGN_UP(sizeof(GraphSubmission), alignof(GraphTensor));
