...
#endif
        if (p_thread_mode_) {
            // 3S+1P: hand the finished task to the dedicated resolution (P)
            // thread. P publishes completion_flags, drains the wake list, and
            // advances the watermark — and owns completed_tasks_, so this
            // scheduler thread neither resolves nor bumps completed_this_turn.
            ...
            if (resolve_t0 != 0) {
                ...
                if (resolve_t1 - resolve_t0 >= RESOLVE_EMIT_MIN_CYCLES) {
                    l2_swimlane_aicpu_record_sched_phase(
                        thread_idx, L2SwimlaneSchedPhaseKind::Resolve, resolve_t0, resolve_t1,
                        l2_swimlane.sched_loop_count, consumers_resolved
                    );
                }
            }
            l2_swimlane.phase_complete_count++;
#endif
            // Polling: on_task_complete published completion + drained the wake
            // list inline; no deferred producer-release step.
            completed_this_turn++;
        }
    }
